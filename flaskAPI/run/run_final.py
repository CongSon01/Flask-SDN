...
# Cesnet200706
# VtlWavenet2011
graph = networkx.read_graphml(
    "/home/onos/Downloads/topologyzoo/"+set_up_topo['name_topo']+".graphml")
nodes_graph = graph.nodes()
edges = [["s"+str(int(edge[0].replace("n", ""))+1),  "s" +
          str(int(edge[1].replace("n", ""))+1)] for edge in graph.edges()]
...
for i in range(len(controllers)):
    ...
    controller_net = net.addController(name=controller_name, controller=RemoteController,
                                       ip=controllers[i]['ip'], protocol='tcp', port=controllers[i]['controller_port'])
    controllers_save[controller_name] = controller_net

not_host = [str(host).replace('s', 'h')
            for host in sum(set_up_topo['bridges'], [])]
...
for edge in edges:
    if edge in set_up_topo["bridges"]:
        print(edge[0], edge[1], " ip: ",
              switches_save[edge[0]].dpid, switches_save[edge[1]].dpid)
        net.addLink(switches_save[edge[0]], switches_save[edge[1]], port1=10, port2=10,
                    delay=LINK_DELAY, loss=LOSS_PER, bw=MAX_CAPACITY_BW, use_htb=True)
        ...
        net.addLink(switches_save[edge[0]], switches_save[edge[1]],
                    delay=LINK_DELAY, loss=LOSS_PER, bw=MAX_CAPACITY_BW, use_htb=True)
    Matrix_graph[int(edge[0].replace("s", "")) -
                 1][int(edge[1].replace("s", ""))-1] = 1
    Matrix_graph[int(edge[1].replace("s", "")) -
                 1][int(edge[0].replace("s", ""))-1] = 1
...
for host_name in hosts_save:
    ...
    net.addLink(hosts_save[host_name], switches_save[name_switch],
                delay=LINK_DELAY, loss=LOSS_PER, bw=MAX_CAPACITY_BW, use_htb=True)
...
map_switch_controller = {
    "c"+str(i): controllers[i]['switches'] for i in range(len(controllers))}
...
def ping_host_in_sdn(net, controllers, not_host):
    for c in controllers:
        hst = ["h" + str(int(switch.replace("s", ""))) for switch in c['switches']
               if "h"+str(int(switch.replace("s", ""))) not in not_host]
